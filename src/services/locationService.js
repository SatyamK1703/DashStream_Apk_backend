...
const _updateProfessionalLocation = async (userId, locationData) => {
  ...
  if (user.role !== "professional") {
    throw new AppError(
      "Only professionals can update their location",
      403,
      ErrorCodes.ROLE_INVALID
    );
  }
...
export const updateProfessionalLocation = withErrorHandling(
...
const _updateProfessionalStatus = async (userId, status) => {
  ...
  if (!location) {
    throw new AppError(
      "Location tracking not initialized for this user",
      404,
      ErrorCodes.LOCATION_NOT_INITIALIZED
    );
  }
...
export const updateProfessionalStatus = withErrorHandling(
...
export const setTrackingEnabled = withErrorHandling(
  _setTrackingEnabled,
  "setTrackingEnabled"
);
...
export const updateTrackingSettings = withErrorHandling(
...
const _getProfessionalLocation = async (professionalId) => {
  ...
  const location = await Location.findOne({ user: professionalId }).populate(
    "user",
    "name phone role isAvailable rating"
  );

  if (!location) {
    throw new AppError(
      "Location not found for this professional",
      404,
...
export const getProfessionalLocation = withErrorHandling(
  _getProfessionalLocation,
...
const _getProfessionalLocationHistory = async (professionalId, limit = 50) => {
  ...
  if (!location.history || location.history.length === 0) {
    throw new AppError(
      ...
      404,
      ErrorCodes.LOCATION_HISTORY_EMPTY
    );
  }
...
export const getProfessionalLocationHistory = withErrorHandling(
  _getProfessionalLocationHistory,
  "getProfessionalLocationHistory"
);

const _findNearbyProfessionals = async (
  coordinates,
  maxDistance = 10000,
  status = "available",
  filters = {}
  ...
  // Generate cache key
  const cacheKey = generateNearbyKey(coordinates, maxDistance, {
    status,
    ...filters,
  });
  ...
  const professionals = await Location.findNearbyProfessionals(
    coordinates,
    distanceInMeters,
    status,
    queryFilters
...
export const findNearbyProfessionals = withErrorHandling(
  _findNearbyProfessionals,
...
const _subscribeToLocationUpdates = async (userId, professionalId) => {
  ...
  if (professional.role !== "professional") {
    throw new AppError(
      "Target user is not a professional",
      400,
      ErrorCodes.ROLE_INVALID
    );
  }
  ...
  if (!location || !location.trackingEnabled) {
    throw new AppError(
      "Location tracking is not enabled for this professional",
      400,
      ErrorCodes.TRACKING_DISABLED
    );
  }
...
export const subscribeToLocationUpdates = withErrorHandling(
...
export const unsubscribeFromLocationUpdates = withErrorHandling(
...
export const getReverseGeocode = withErrorHandling(
  _getReverseGeocode,
  "getReverseGeocode"
);
...
export const notifyLocationSubscribers = withErrorHandling(
  _notifyLocationSubscribers,
