...
// Session configuration (use memory store during tests to avoid DB dependency)
  ...
  app.use(
    session({
      ...
      cookie: {
        ...
        httpOnly: true,
        maxAge:
          parseInt(process.env.SESSION_EXPIRY) || 14 * 24 * 60 * 60 * 1000, // 14 days by default
...
if (process.env.NODE_ENV !== "test") {
  ...
  mongoose
    ...
    .catch((err) => {
      ...
      console.log(
        "Please make sure MongoDB is installed and running, or update the MONGODB_URI in .env file."
      );
    });
